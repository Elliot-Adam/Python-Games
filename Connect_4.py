...
def colors(color) -> str:
    ...
    global color_og_list
    global color_dict
    
    ret_value = color_dict[color]

    color_og_list.remove(color_dict[color])
    color_dict.clear()
    for key,value in enumerate(color_og_list,start= 1):
        color_dict[str(key)] = value
    return ret_value
...
def var_defs() -> None:
    """Defines all necessary variables to later bits of the code"""
    global color_list
    global board
    global color_og_list
    global color_dict
